...
CREATE POLICY "Students view assigned version questions" ON public.questions FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.test_attempts a WHERE a.version_id = questions.version_id AND a.student_id=auth.uid()));
...
CREATE TABLE public.attempt_responses (
  ...
  state text NOT NULL DEFAULT 'visited' CHECK (state IN ('not_visited','visited','answered','review','answered_review','cleared','skipped')),
...
CREATE POLICY "Students manage own responses" ON public.attempt_responses FOR ALL TO authenticated USING (EXISTS (SELECT 1 FROM public.test_attempts a WHERE a.id=attempt_id AND a.student_id=auth.uid())) WITH CHECK (EXISTS (SELECT 1 FROM public.test_attempts a WHERE a.id=attempt_id AND a.student_id=auth.uid()));
